...
#show: doc.with(
  ...
  conclusion: [
    ...
    本项目工作的局限性与未来改进方向包括：当前处理器仍为单发射结构，指令吞吐率受限于每周期至多提交一条指令，后续可扩展为双发射或多发射以进一步提升性能，届时 BTB 与多端口 BP 也将成为必要的补充；当前前端将 Predict 模块挂在 F3 的组合路径上，这在未来追求更高时钟频率时会成为时序关键路径，可通过将 BPU 拆出独立流水段缓解；当前缓存层次仅有 L1，可增加 L2 缓存以降低主存访问延迟；乘除法单元采用多周期迭代实现，可替换为流水化设计以提升吞吐率。此外，可进一步完成 FPGA 综合与上板验证，评估实际时钟频率与资源占用。本项目已在 GitHub 上以 GPLv2 协议开源 #footnote[项目仓库：#link("https://github.com/KINGFIOX/ysyx-workbench")[https://github.com/KINGFIOX/ysyx-workbench]]。
  ],
...
+ 完成基于虚拟索引物理标记（VIPT, Virtually Indexed Physically Tagged）#cite(<cekleov1997vipt>) 结构且采用伪最近最少使用（PLRU, Pseudo Least Recently Used）替换策略的指令缓存（ICache, Instruction Cache）与数据缓存（DCache, Data Cache）设计与实现。
+ 完成 RISC-V M/S/U 三级特权架构与 Sv39 虚拟内存机制实现 #cite(<riscv2024priv>)，具备操作系统运行所需的体系结构支持能力。
+ 完成 NVBoard（NJU Virtual Board）#cite(<nvboard2024>) 集成，实现仿真平台的外设可视化。
+ 完成 xv6 #cite(<cox2024xv6>) 教学操作系统移植与运行验证。
+ 基于混合性能计数器（RTL 寄存器 + DPI-C 事件）完成 MicroBench #cite(<microbench2024>)、Dhrystone #cite(<weicker1984dhrystone>) 与 CoreMark #cite(<coremark2009>) 三组负载的性能评估，三组负载 IPC 达 0.43–0.56，Dhrystone 成绩 1.596 DMIPS/MHz、CoreMark 成绩 1.540 CoreMark/MHz。
...
在仿真验证方面，参照 ISSI IS66WVS4M8ALL/BLL 手册 #cite(<issi_is66wvs4m8>) 中的命令字与时序规范，实现了 PSRAM 颗粒的行为级仿真模型，用于在 Verilator 仿真环境中验证 QSPI 控制器的协议正确性。

== SDRAM 控制器

为提供较大容量的主存储空间，SDRAM 控制器挂载在一级 AXI4 Crossbar 上。本课题参考 ultraembedded 开源的 SDRAM 控制器 #cite(<ultraembedded_sdram>)（Verilog 实现），使用 Chisel 进行了完整重写，以实现参数化扩展并与 SoC 其他 Chisel 模块统一维护。重写后的控制器利用 Chisel 内置的 Queue 模块对请求和响应通路进行解耦，流水化程度高于原始 Verilog 版本。
...
F1 承担取指侧的地址翻译职责，是前端事实上的 MMU 级，其典型实现即本项目的 `F1Stage` 子模块。模块内部由一个 16 项全相联的 iTLB 与一个 PTW 子 FSM 组成：iTLB 负责命中时的快路径，PTW 负责缺失时的慢路径回填；iTLB 的替换策略采用位矩阵 LRU（Bit-Matrix LRU）算法 #cite(<belady1966replacement>)，详见 @sec:tlb。Sv39 未启用时 F1 直接将虚拟地址低位截断作为物理地址组合放行；启用后则对 PC 的 VPN 做一次组合查表，命中且 PTE.X 合法则一拍内输出翻译后的物理地址，命中但 PTE.X 不合法则置位 `page_fault` 标志并向 F2 输出 mcause = 12 的指令页错误；若 TLB 缺失，则启动 PTW 沿 `ptw_port` 走三级页表，完成后回填 iTLB 并在下一拍重查。

PTW 子 FSM 是整个 F1 中唯一的状态机，需要与后文 @sec:ptw 中的 PTW 内部九态遍历 FSM 加以区分：前者是 F1Stage 内部用于驱动 PTW 请求/响应握手并与上层 squash 信号协同的外层 FSM，后者才是真正完成三级页表遍历的内部算法状态机。由于 F0 的投机推进在任意一拍都可能被 taken 分支或后端 flush 打断，PTW 必须能在"已经向 AXI 发起页表读"的中途被 squash 而不留下孤悬事务，否则后续 TLB 缺失会因 `ptw.req.ready = 0` 而死锁。为此 F1 内部维护 @fig:ifu-ptw-fsm 所示的四态握手 FSM，并配合两个辅助寄存器 `ptw_vpn_reg` 与 `ptw_abandoned` 记录当前在途请求的上下文。
...
本项目实现了 RISC-V Sv39 虚拟内存方案 #cite(<riscv2024priv>)。Sv39 将 64 位虚拟地址的低 39 位划分为三级页号（VPN[2:0]，每级 9 位）与 12 位页内偏移，通过三级页表将虚拟页号翻译为 44 位物理页号（PPN）。当 satp 寄存器的 MODE 字段为 8 时启用 Sv39，satp.PPN 指向根页表的物理基地址。
...
上一节讨论的"指令-数据一致性"限定在单个 CPU 内部 ICache 与 DCache 之间；当处理器外部存在第二个主设备时，一致性问题会从"同一 core 内两份缓存"扩展到"核心侧 DCache 与外部主设备所见的主存"之间。本项目在移植 xv6 时接入了 virtio-disk 控制器，其数据块传输由 DMA 引擎自行完成：DMA 引擎与 CPU 一同挂载到 AXI4 Crossbar 上（见 @fig:soc-arch），以周期窃取（cycle stealing）的方式共用通往 SDRAM 的访存通道——CPU 未占用总线的拍次由 DMA 填入，二者对 Crossbar 而言是对称的主设备。
...
性能评估在本项目基于 Verilator 构建的 C++20 仿真平台上运行，所有被测程序均以 rv64im_zicsr_zifencei #cite(<riscv2024unpriv>) 为目标架构交叉编译，运行于 Abstract Machine #cite(<nju_abstract_machine>) 抽象层提供的裸机环境中，不经操作系统调度。仿真平台以 VL\_USER\_STOP 退出策略在程序执行 `halt` 后终止并产生计数器报告。
...
+ 缓存访问、缓存命中、缓存未命中、缓存缺失周期（Miss Cycles）、IFU 输出有效、IFU 停顿等事件则通过一个仅用于仿真的 SystemVerilog #cite(<ieee2017systemverilog>) 外部模块 `PerfEventHelper` 在每个触发时钟沿调用 DPI-C #cite(<ieee2017systemverilog>) 函数 `npc_perf_event(id)`，由 C++ 侧的 `PerfCounters` 单例累加。该方案不在 RTL 中添加任何额外寄存器或状态机逻辑，故对综合面积与关键路径无影响。

这种"寄存器 + 事件"的划分与业界主流开源处理器的性能监控单元（PMU, Performance Monitoring Unit #cite(<arm2024armArm>) #cite(<wang2023xiangshan>)）设计思路一致：需要被软件读取或需要多周期聚合的大计数器保留在 RTL 中，单次脉冲型事件则通过 DPI-C 导出到仿真平台，避免为仿真观测而污染处理器数据通路。
