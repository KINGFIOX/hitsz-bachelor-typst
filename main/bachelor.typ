...
\2. 基于 Chisel #cite(<bachrach2012chisel>) 完成关键知识产权核（IP, Intellectual Property Core）实现，包括 SDRAM 控制器、Flash 控制器（串行外设接口（SPI, Serial Peripheral Interface）模式，配合就地执行（XIP, eXecute In Place）控制器实现直接取指执行）与 PSRAM 控制器（支持四线串行外设接口（QSPI, Quad SPI）与四线并行 I/O（QPI, Quad Peripheral Interface）模式及相互切换）。

\3. 完成单发射乱序处理器设计，采用统一物理寄存器重命名方案，实现乱序执行、顺序提交与精确异常支持。

\4. 完成基于物理索引物理标记（VIPT, Virtually Indexed Physically Tagged）结构且采用伪最近最少使用（PLRU, Pseudo Least Recently Used）替换策略的指令缓存（ICache, Instruction Cache）与数据缓存（DCache, Data Cache）设计与实现。
...
\8. 基于混合性能计数器（RTL 寄存器 + DPI-C 事件）完成 MicroBench / Dhrystone / CoreMark 三组负载的性能评估，三组负载 IPC 达 0.43–0.56，Dhrystone 成绩 1.596 DMIPS/MHz、CoreMark 成绩 1.540 CoreMark/MHz。
...
为在保持单发射单提交架构约束的前提下充分释放后端乱序执行潜能，IFU 采用 4 段流水线结构，目标是在直线代码上达到稳态 1 IPC，taken 分支仅付出 2 个气泡周期。流水结构同时需要兼顾 Sv39 地址翻译的慢路径（TLB 缺失 → PTW）以及 ICache 不定长填充延迟，这两类不定长事件的协调是流水化相对于朴素状态机取指的主要工程难点。

=== 流水段划分

IFU 分为四个主段与一个辅助段，如 @fig:ifu-pipeline 所示：
  ...
  #figure(
    scale(
      ...
      render(
        ...
        Fp [label=\"Fp\\n(in-flight)\"];
        F3 [label=\"F3\\nResp+Predict\"];
        BE [label=\"Back-end\", shape=ellipse];
        PTW [label=\"PTW\\nsub-FSM\", shape=ellipse, style=dashed];

        F0 -> F1;
        F1 -> F2;
        F2 -> Fp [label=\"req accepted\"];
        Fp -> F3 [label=\"rdata ready\"];
        F3 -> BE [label=\"io.out.fire\"];
        F1 -> PTW [style=dashed, label=\"TLB miss\"];
        PTW -> F1 [style=dashed, label=\"refill / fault\"];
        F3 -> F0 [label=\"fe_redirect\", style=dashed, color=\"red\"];
        BE -> F0 [label=\"io.flush\", color=\"blue\"];
      }",
      ),
    ),
    caption: [4 段流水化后的 IFU 结构],
    supplement: [图],
  ) <ifu-pipeline>
]

+ F0（PC Gen）：维护架构 PC 寄存器 `pc_q`，每拍假设下一条指令为 `pc_q + 4` 并推进 F1。基于"直线代码占绝大多数"的统计特征，采用投机顺序推进；一旦 F3 确认出 taken 分支或 serializing 指令，再由前端自重定向回卷。
+ F1（iTLB Lookup）：对 PC 的 VPN 执行 iTLB 组合查表。命中时一拍放行；Sv39 禁用时绕过查表直取物理地址；TLB 缺失则将请求交给 PTW 子 FSM 慢路径走三级页表，完成后回填 iTLB。
+ F2（ICache Req）：向 ICache 发起读请求，握手 `icache.req / icache.ack`。ICache 实现了 lookup+hit 同拍接受新 req 的能力，使得连续命中时 F2 每拍都能放行。
+ Fp（Cache in-flight）：记录一个已被 ICache 接受但数据尚未返回的请求，将 `icache.rdata` 锁存到 `fp_inst_buf` 后再平滑地递交给 F3，避免 F3 短暂反压时数据丢失。
+ F3（Resp + Predict + Handoff）：指令字最终可见；Predict 模块在此拍组合产生预测 `dnpc` 与 GHR 快照；通过 `io.out` 与后端指令队列握手。

=== 前端自重定向

单纯放开流水反压会让后端每次 taken 分支都承担"整条取指流水清空 + 重填"的完整代价。为此在 F3 引入前端自重定向 `fe_redirect`：当 F3 交付指令的 `predict_dnpc` 与其 `pc + 4` 不一致时，同周期将 F0/F1/F2/Fp 全部清空并把 `pc_q` 重置为 `predict_dnpc`。这样 taken 分支对下游只表现为 2 个气泡周期（F3 空两拍），而不是完整 4 拍流水清空。

对于 fence.i、ecall/ebreak、mret/sret、sfence.vma 等必须等后端权威 PC 的 serializing 指令，则不仅自重定向，还置一位粘贴位 `fe_wait_flush`，让 F0 停止发射，直到后端的 `io.flush` 到来携带正确 PC。需要特别注意的是，普通 CSR 读写（funct3 $!=$ 000）在本处理器后端并不触发 `io.flush`，若将其归入 serializing 集合会导致前端永远停摆；实现上以 opcode 与 funct3 精确区分 CSR 和系统指令，避免这一死锁。

=== PTW 与 Cache 协同带来的新增复杂性

流水化后，前端任意一段都可能在同一时刻持有"正在飞行"的状态，这使得 squash 时机与下游延时的耦合显著复杂化。主要工程难点包括：

+ ICache 失效与投机取指的竞争：AXI4 ICache 仅在 `IDLE` 状态接受 `fence_i` 无效脉冲。若 F3 即将提交 fence.i，而 F0–Fp 已在总线上并发取下一行，ICache 正处于 refill 状态时 `fence_i` 脉冲会被吞掉，导致旧行残留。通过 `fe_wait_flush` 机制，fence.i 提交时同周期 squash 整条取指流水并阻塞 F0，使 ICache 在后端 flush 到达前始终处于 IDLE，保证 `fence_i` 脉冲被正确采样。
+ PTW 子 FSM 与 squash 的协同：PTW 在 AXI 上发起三级页表读，其中任意一拍都可能被 `fe_redirect` 或 `io.flush` 打断。若直接把 IFU 内部的 PTW 跟踪状态归零而 PTW 模块本身仍在总线上走页表，后续新的 TLB 缺失就会因 `ptw.req.ready=0` 而死锁。为此实现中：
  - 在 IFU 侧维护 `idle / req / waitResp / drain` 四态子 FSM；
  - 为 inflight 的请求记录 `ptw_vpn_reg` 与 `ptw_abandoned` 两个寄存器；
  - squash 发生时若 PTW 处于 waitResp 则转入 drain，默默吞掉即将到来的响应而不刷 iTLB；
  - 回填与 fault 上报均校验 `f1_vpn === ptw_vpn_reg`，防止 squash+新 F1 交错导致的错填。
+ RAS 推测路径污染防护：若 RAS 的 push/pop 挂在 F3 指令出现的时刻，一旦 F3 被反压或被 squash，就会出现 call/ret 配对错乱。实现中把 RAS 的 push/pop 严格挂到 `io.out.fire`（即 F3 向后端成功交付）上，保证只有真正提交给后端的 call/ret 才影响 RAS 状态。
...
本课题的设计目标是在尽可能简单的前端实现下取得合理的预测覆盖。在本项目的 4 段流水前端（@sec:ifu-pipeline）中，预测器工作在 F3 阶段——指令字已在 F3 可见，指令类型与立即数可得。对于 JAL 和 BRANCH 这两类指令，目标地址为 PC + 立即数的简单加法，可在当拍直接计算得出，无需额外的缓存结构；仅对 JALR 这类目标地址依赖寄存器值、前端无法计算的指令，才借助 IJTC 或 RAS 进行预测。因此在本微架构下，BTB 的作用被直接计算和专用预测器所替代，省去了 BTB 的存储开销与维护逻辑；后续若进一步演进为多发射或双发射，则 BTB 才会成为必要的补充。
...
本章介绍了前端的取指单元与动态分支预测设计。IFU 采用 F0、F1、F2、（Fp）、F3 的 4 段流水结构，在保持单发射单提交约束的前提下，以"直线代码稳态 1 IPC、taken 分支仅引入 2 拍气泡"为性能目标；同时通过前端自重定向、PTW 子 FSM 与 RAS 推测保护等机制，妥善收敛了 fence.i 与页表遍历等复杂场景。分支预测采用 GShare + IJTC + RAS 组合方案，分别对应条件分支、间接跳转与函数返回三类控制流。在目标地址计算上，得益于前端已完成预解码，JAL 与 BRANCH 的目标地址可直接由 PC 与立即数相加得到，仅 JALR 需要借助专用预测器，从而省去了 BTB 的存储与维护开销。在更新策略上，GShare 与 IJTC 采用保守策略，仅在指令提交时更新，以避免推测路径对预测器状态造成污染；RAS 则挂在 `io.out.fire` 上，即在指令真正交付给后端的时刻完成 push/pop，从而利用 call/ret 成对出现的特性降低预测延迟。在 MicroBench 测试套件 test 数据规模下（如 @fig:microbench-test 所示），处理器共提交 653614 条指令，其中 156330 条分支指令产生 20263 次预测错误，分支预测命中率达到 87.0%，验证了 GShare + IJTC + RAS 组合方案在实际工作负载下的有效性。
...
#figure(
  ...
  align(center)[
    #table(
      ...
      table.header([基准], [fe\_redirect branch], [fe\_redirect serial], [F0 wait\_flush], [F1 TLB miss], [F2 ICache wait], [Fp resp wait]),
      [MicroBench (test)], [70 195], [46], [482], [0], [182 963], [5 970],
      [Dhrystone (2 000)], [161 648], [33], [472], [0], [403 268], [1 825],
      [CoreMark (20 iter)], [856 609], [482], [2 538], [0], [1 187 045], [3 996],
    )
  ],
) <tab:perf-fe-events>
...
+ 前端自重定向的代价可控：所有负载上 `fe_redirect (serial)` 的次数均远小于 taken 分支的次数（以 CoreMark 为例，两者分别为 482 与 856 609），说明 fence.i、ecall、sret 等需要等待后端回传权威 PC 的 serializing 指令在实际负载中出现得极为稀疏，不足以成为 IPC 瓶颈；相应地，`F0 wait_flush` 累计阻塞的周期数也仅占总仿真周期的万分之几。
...
+ 前端事件分解显示，前端停顿主要来自 F2 阶段的 ICache 等待（占总周期的 9% 左右），而 fe\_redirect / F0 wait\_flush 等自重定向开销仅占万分之几，说明 fence.i / 页表遍历等 serializing 场景的实现是高效且稳健的；
