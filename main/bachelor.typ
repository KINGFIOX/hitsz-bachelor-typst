...
#figure(
  image("images/microbench-test.png", width: 80%),
    ...
    scale(70%,
      render("
        digraph BackEnd {
          ...
          node [shape=box, style=rounded, fontsize=9, height=0.35];
          edge [fontsize=7, arrowsize=0.6];

          IQ [label=\"Instruction Queue\", shape=Mrecord];
          ...
          subgraph cluster_be {
            ...
            REN -> FRAT [label=\"read/write\", dir=both, style=dashed];
            REN -> FL [label=\"alloc\", style=dashed];
            REN -> BT [label=\"set busy\", style=dashed];
            DISP -> ROB [label=\"enq\", style=dashed];
            ...
            DISP -> MDU_IQ;
            DISP -> BRU_IQ;
            DISP -> PRF [label=\"imm write\", style=dashed];
            ...
            ALU_IQ -> ALU [label=\"issue\"];
            MDU_IQ -> MDU [label=\"issue\"];
            BRU_IQ -> BRU [label=\"issue\"];
            ROB -> LSU [label=\"late exec\"];
            ROB -> CSRU [label=\"late exec\"];

            ALU -> PRF [label=\"write\"];
            MDU -> PRF [label=\"write\"];
            LSU -> PRF [label=\"write\"];
            CSRU -> PRF [label=\"write\"];
            ALU -> ROB [label=\"wb\"];
            MDU -> ROB [label=\"wb\"];
            BRU -> ROB [label=\"wb\"];

            ALU_IQ -> BT [label=\"read\", style=dashed, dir=both];
            MDU_IQ -> BT [label=\"read\", style=dashed, dir=both];
            BRU_IQ -> BT [label=\"read\", style=dashed, dir=both];

            COMMIT [label=\"CommitStage\"];
            ROB -> COMMIT [label=\"commit\"];
            COMMIT -> ARAT [label=\"write\", style=dashed];
            COMMIT -> FL [label=\"free old_prd\", style=dashed];
          }

          IQ -> DEC;

          ARAT -> FRAT [label=\"flush snapshot\", style=dashed, constraint=false];
          ARAT -> FL [label=\"flush snapshot\", style=dashed, constraint=false];

          {rank=same; FRAT; ARAT; FL; BT; PRF; ROB}
          {rank=same; ALU_IQ; MDU_IQ; BRU_IQ}
          {rank=same; ALU; MDU; BRU; LSU; CSRU}
        }
      ")
    ),
    caption: [后端乱序执行引擎整体架构],
    supplement: [图],
  ) <backend-arch>
]

如 @fig:backend-arch 所示，后端流水线由译码（Decode）、重命名（Rename）、分派（Dispatch）三个顺序级和多条并行执行路径组成。指令在重命名级完成逻辑寄存器到物理寄存器的映射后，由分派级写入 ROB 并根据指令类型送入对应的发射队列或直接写回结果。执行完成后通过 ROB 顺序提交，提交时更新 ArchRAT 并释放旧物理寄存器。
...
寄存器重命名是乱序执行的核心前提，其目的是消除指令间由逻辑寄存器复用导致的 WAW（写后写）与 WAR（写后读）假依赖，使真正的数据依赖（RAW）成为唯一的调度约束。本文采用统一物理寄存器文件（Unified Physical Register File）方案，主要涉及以下结构：
...
*FreeList*。FreeList 管理空闲物理寄存器池。重命名级从中分配，提交级向其归还。当发生 flush 时，FreeList 根据 Arch RAT 快照重建：凡不在 Arch RAT 映射中的物理寄存器均标记为空闲，一拍即可完成恢复。

*BusyTable*。BusyTable 记录每个物理寄存器的数据就绪状态。重命名级在分配新物理寄存器时将其标记为 busy；执行单元写回结果时通过 wakeup 信号清除 busy 标记。发射队列在发射前查询 BusyTable 以判断源操作数是否就绪。BusyTable 配置 6 个读端口供三个发射队列使用（每队列 2 个，对应 rs1 和 rs2），5 个 wakeup 端口分别接收来自 ALU、MDU、分派级立即数写入、LSU 和 CSRU 的就绪通知。
...
\1. 将指令写入 ROB，获取 ROB 标签（rob\_tag）作为指令在乱序管线中的唯一标识。
...
后端设置三个独立的发射队列，分别服务于 ALU、MDU 和 BRU 执行单元。每个发射队列在入队时记录指令的源物理寄存器号与 ROB 标签，在发射前通过 BusyTable 的读端口查询两个源操作数的就绪状态。当源操作数全部就绪时，指令即可被选中发射。由于多条指令可能同时就绪，发射队列内部通过优先级编码器选择最早入队的指令发射，保证公平性。
...
*LSU 路径*。处理 Load/Store 指令。与 ALU/MDU/BRU 不同，LSU 采用延迟执行（late execution）模式：指令在分派时仅写入 ROB，待 ROB 将其送至提交头部附近时才通过专用端口交给 LSU 执行。这一设计确保访存操作按程序顺序执行，简化了存储一致性维护。LSU 内部连接 DCache 与外设端口，并集成 dTLB 与 PTW 以支持 Sv39 地址翻译。LSU 写回通过独立的 PRF 写端口与 wakeup 通路完成。

*CSRU 路径*。处理 CSR 读写指令。与 LSU 类似采用延迟执行模式，由 ROB 在提交阶段交给 CSRU 执行，保证 CSR 操作的顺序语义。CSRU 同时负责中断与异常的状态管理，包括 trap 入口计算、mret/sret 返回地址提供、中断挂起检测等。
