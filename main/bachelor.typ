...
F1 承担取指侧的地址翻译职责，是前端事实上的 MMU 级，其典型实现即本文的 `F1Stage` 子模块。模块内部由一个 16 项全相联的 iTLB 与一个 PTW 子 FSM 组成：iTLB 负责命中时的快路径，PTW 负责缺失时的慢路径回填；iTLB 的替换策略采用位矩阵 LRU（Bit-Matrix LRU）算法 #cite(<belady1966replacement>)，详见 @sec:tlb。Sv39 未启用时 F1 直接将虚拟地址低位截断作为物理地址组合放行；启用后则对 PC 的 VPN 做一次组合查表，命中且 PTE.X 合法则一拍内输出翻译后的物理地址，命中但 PTE.X 不合法则置位 `page_fault` 标志并向 F2 输出 mcause = 12 的指令页错误；若 TLB 缺失，则启动 PTW 沿 `ptw_port` 走三级页表，完成后回填 iTLB 并在下一拍重查。
...
为使类 SRAM 总线能够融入 Rocket Chip #cite(<asanovic2016rocket>) 的 Diplomacy 参数协商框架，本文为其实现了完整的节点体系。Diplomacy 框架的核心思想是：总线参数（地址宽度、数据宽度、地址映射等）不在模块实例化时硬编码，而是由连接图中的上下游节点在编译期自动协商确定。
...
性能评估在本文基于 Verilator 构建的 C++20 仿真平台上运行，所有被测程序均以 rv64im_zicsr_zifencei #cite(<riscv2024unpriv>) 为目标架构交叉编译，运行于 Abstract Machine #cite(<nju_abstract_machine>) 抽象层提供的裸机环境中，不经操作系统调度。仿真平台以 VL\_USER\_STOP 退出策略在程序执行 `halt` 后终止并产生计数器报告。
...
+ Dhrystone 达到 1.596 DMIPS/MHz（已跨越 Cortex-M3 档位），CoreMark 达到 1.540 CoreMark/MHz，验证了本文所实现的处理器在典型嵌入式/教学基准上的性能水平。
