...
TLB 采用 16 项全相联结构，以 VPN（27 位）作为 tag 进行匹配查询。查询时对所有表项并行比较，通过优先级编码器选中命中项，返回对应的 PPN 与页表标志位（flags）。TLB 的替换策略采用简单的轮转指针（round-robin）：每次 refill 写入 replacePtr 指向的表项并递增指针。sfence.vma 指令执行时，通过 flush 信号一次性失效全部表项。
