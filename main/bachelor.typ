...
分支预测能力直接决定前端取指带宽的有效利用率。相关技术从静态预测与一/二位饱和计数器，发展到两级自适应预测（如 GShare、GSelect）#cite(<mcfarling1993combining>)，再到混合预测与锦标赛选择机制 #cite(<razilov2024tage>)。同时，分支目标缓冲器（BTB, Branch Target Buffer）、返回地址栈（RAS, Return Address Stack）与间接跳转目标缓存等结构被联合用于覆盖条件分支、函数返回与间接跳转场景。随着乱序窗口扩大与流水线加深，分支预测准确率与误判恢复代价已成为影响处理器实际吞吐率的关键因素。
...
\4. 完成基于物理索引物理标记（PIPT, Physically Indexed Physically Tagged）结构且采用伪最近最少使用（PLRU, Pseudo Least Recently Used）替换策略的指令缓存（ICache, Instruction Cache）与数据缓存（DCache, Data Cache）设计与实现。
...
前端负责为后端持续供应有效指令，其内部由取指单元（IFU, Instruction Fetch Unit）与分支预测单元（Predict）两个子模块组成。IFU 通过状态机驱动完成地址翻译与 ICache 访问，Predict 对当前取到的指令进行轻量译码并输出预测的下一条指令 PC，两者协同工作形成"取指—预测—下一轮取指"的闭环。
