...
+ CPU $arrow$ DMA 方向：xv6 在发起磁盘写请求前，先将待写入的数据填入驱动描述符指向的数据缓冲区；这条 `sw` 经 DCache 采用的写回策略被吸收为脏行，尚未通过 AXI4 写通道落回 SDRAM。此时若 CPU 继续向 virtio 的 MMIO 通知寄存器写入 "queue notify"，DMA 便会立即从 SDRAM 读取数据——读到的却是 DCache 背后的旧值，磁盘最终落盘的是错误内容。
+ DMA $arrow$ CPU 方向：磁盘读完成后 DMA 将数据直接写入 SDRAM 的目标缓冲区，并通过中断或 virtio 的 used ring 通知 CPU。若 CPU 在此前曾访问过同一地址范围，DCache 中已缓存了旧数据，后续的 `lw` 将命中 DCache 而根本看不到 DMA 刚刚写入的新数据，内核读到的依然是未更新前的内容。

两条路径共同揭示的问题是：DMA 事务的可见边界是 SDRAM，而 CPU 侧的观察窗口是 DCache；只要中间的写回/失效步骤未被显式触发，两方观察到的主存快照就会错位。

由于 xv6 的 virtio 驱动是本项目自行移植并适配的，本项目从软件侧对这一问题做了显式规避：在所有会触发或终结 DMA 事务的 MMIO 访问前后各插入一条 fence 指令。前端一侧的 fence 借助 DCache 主状态机的 `flush_scan -> flush_wb` 路径（见 @fig:dcache-fsm）将当前所有脏行写回 SDRAM，确保 CPU 已写入的数据对 DMA 可见；后端一侧的 fence 则同时起到作废 DCache 全部行的作用，迫使后续 `lw` 重新从 SDRAM 读取 DMA 刚刚刷入的内容，从而覆盖 DMA $arrow$ CPU 方向的可见性缺口。因 fence 的粒度是整个 DCache，配合 xv6 中 MMIO 访问频次不高的事实，这一软件方案在正确性上足以支撑 xv6 启动并稳定运行用户态程序，同时避免了在硬件上引入任何跨主设备的同步结构，复杂度代价最低。

这套以全局 fence 换取正确性的策略并非没有代价：每次磁盘块传输都触发一次整 cache 刷写，DCache 的时间局部性被完全击穿，对磁盘访问密集型负载（如文件系统压测、Shell 启动）的性能表现会显著恶化。更理想的做法是在硬件侧引入跨主设备的缓存一致性协议，例如类似 MESI 的嗅探（snoop）机制：DMA 发起访存时，Crossbar 以 bus snooping 将事务广播至 DCache，由 DCache 根据本地 tag 与行状态决定是转发脏数据、降级本地状态还是直接放行；CPU 写命中共享行时主动发出 invalidate 广播，使外部主设备感知。这样软件就无需在每次 MMIO 前后粘贴 fence，一致性由硬件按行粒度自动维护，对缓存局部性的损伤也被限制在真正发生冲突的单个缓存行上。这一方向涉及 AXI4-ACE/CHI 等一致性互连协议与多主设备仲裁的重新设计，已超出本项目单核教学目标的范围，留作未来工作。
...
性能评估在本项目基于 Verilator 构建的 C++20 仿真平台上运行，所有被测程序均以 rv64im_zicsr_zifencei 为目标架构交叉编译，运行于 Abstract Machine 抽象层提供的裸机环境中，不经操作系统调度。仿真平台以 VL\_USER\_STOP 退出策略在程序执行 `halt` 后终止并产生计数器报告。
