...
F0 位于 IFU 顶层，不作为独立子模块存在，只维护架构 PC 寄存器 `pc_q`。在每个周期，F0 都投机地按 `pc_q + 4` 推进到 F1。若 F3 确认出现 taken 分支（如直接跳转、被预测为跳转的条件分支等），前端会立即执行自重定向，将 `pc_q` 拉回预测目标。除此之外，凡是需要改写 PC 的情况（分支误预测、trap、xret、`fence.i`、`sfence.vma`、CSR 写等），统一由后端通过 `io.flush` 携带正确的 `io.dnpc`(Dynamic Next Program Counter, dnpc) 来重写 `pc_q`。顶层还会生成全局 squash 信号 `squash = fe_take_branch ⋁ io.flush`，并广播到所有 `PipelineConnect` 的 `flush` 端口，以及 F1 与 Fp 的内部状态寄存器，从而保证整条流水在同一拍内同步清空。F1、F2+Fp、F3 三段的具体实现见下文。
...
F2 是纯组合段，不持有寄存器。它根据 `F2In` 驱动 `req / addr / size` 并完成 `req / ack` 握手；地址按指令字长对齐，请求仅在“上游有效、非故障、且 Fp 有空槽”时发起，避免无效占用 ICache 带宽。对 F1 上报的 page fault（页故障），F2 走“故障旁路“直送 F3、绕过 Fp（故障不应占用 inflight 槽位）。“cache 路径“与“故障旁路“最终由“故障旁路“优先 Mux 汇合为同一份 `F3In`，再经 `PipelineConnect` 进入 F3。

Fp 是 IFU 中承载多条 inflight cache 读的核心结构，实现为深度 4 的环形队列。采用 4 深度而非单项缓冲，是为了匹配 ICache 连续命中时“每拍可接收新请求”的流水能力，避免 F2 过早反压。Fp 使用三个指针：`enq_ptr`（F2 入队）、`deq_ptr`（F3 出队）和 `rdata_ptr`（响应回填）；前两者通过翻转位区分满/空，`rdata_ptr` 在每次 `icache.done` 时写入 `slots(rdata_ptr)`。
...
在取指流水化方面，IFU 采用 F0、F1、F2、Fp、F3 的流水结构，以"直线代码稳态 IPC=1、taken 分支仅引入 2 拍气泡"为性能目标。F1 作为 MMU 级完成 iTLB 查询与 PTW 慢路径遍历；F2 与 Fp 共同构成 ICache 控制级，F2 负责请求通道、Fp 以深度 4 的环形 FIFO 追踪在飞响应，二者间以直连代替 `PipelineConnect` 以满足 ICache 握手时序；F3 作为提交点在一拍内完成指令切分、预测驱动与后端交付。
...
RISC-V 指令集架构通过 `fence.i` 指令显式解决这一问题。正确的指令序列如 @tbl:fencei-correct-seq 所示：
...
软件侧 UART 初始化如 @fig:uart-init 所示。这里 soc 侧的分频系数 `div = 1`，只需让 NVBoard 满足相同的采样频率即可正常收发字符。
