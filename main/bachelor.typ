...
本章介绍了前端的取指单元与动态分支预测设计。IFU 采用七状态 FSM 适配 Sv39 地址翻译与 ICache 不定长延迟；分支预测采用 GShare + IJTC + RAS 组合方案，分别覆盖条件分支、间接跳转与函数返回三类控制流场景。在目标地址获取上，利用前端已完成预解码的特点对 JAL 与 BRANCH 直接计算目标地址，仅对 JALR 依赖专用预测器，省去了 BTB 开销。在更新策略上，GShare 与 IJTC 保守地在指令提交时更新以避免推测污染，RAS 则激进地在前端即时更新以利用 call/ret 配对特性降低预测延迟。
