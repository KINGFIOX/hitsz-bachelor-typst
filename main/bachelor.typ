...
三组负载的 IPC 分别为 0.47、0.45 与 0.55，CPI 对应为 2.12、2.21 与 1.81；IPC 随负载特性分层出现，其中 CoreMark 由于存在较多长直线段和极高的 ICache 命中率（见 @tab:perf-icache），可被流水前端连续供应指令，因而取得最高的 IPC。MicroBench train 与 Dhrystone 的 IPC 接近（0.4714 vs 0.4522），两者差距主要来自前端停顿比例与分支误预测惩罚的不同。
...
整体而言，本设计的 GShare + IJTC + RAS 组合在三组负载上的命中率均处于 90% 以上的区间，与同类配置的 GShare 在 SPEC CPU2006 #cite(<henning2006spec2006>) 整数子集上 88%–92% 的平均命中率水平相当，符合主流教学及研究实现的预期。
