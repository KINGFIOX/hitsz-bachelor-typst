...
#figure(
  image("images/boot-flow.png", width: 50%),
  caption: [启动链路流程图],
  supplement: [图],
) <boot-flow>
...
在验证方法上，SoC 级联调与指令级差分测试#footnote[详见后文 @difftest 。]协同使用：差分测试用于保证体系结构状态正确性，NVBoard#footnote[详见后文 @nvboard-vis 。] 与外设交互用于暴露系统级时序和接口问题，二者互补提升了复杂问题定位效率。基于该思路，系统能够稳定支撑操作系统运行与外设协同工作。
...
系统为 Flash 提供两种访问方式。第一种是命令方式：通过 APB Fanout 经 SPI Arbiter 直接读写 SPI 控制器寄存器，适用于 Bootloader 阶段的 Flash 编程与配置操作，如 @flash-read-sw 所示。第二种是 XIP（eXecute In Place）方式：XIP 控制器将处理器对 Flash 地址窗口的 MMIO 读请求自动转换为底层 SPI 命令字序列（如读命令、地址发送与数据回传），从而以硬件方式完成“取指驱动”，其状态机流程如 @fig:xip-fsm 所示。引入该机制的动机是避免启动阶段的“鸡生蛋”死锁：若程序存放在 Flash 中而访问 Flash 又依赖软件驱动，则驱动本身也需要先从 Flash 取出，系统将无法启动。通过 XIP 的硬件读通路，处理器无需先运行 SPI 软件驱动即可直接从 Flash 取指执行。两种方式通过 SPI Arbiter 共享物理 SPI 控制器，XIP 具有更高的仲裁优先级。
...
为此，F3 额外提供一条基于预测结果的前端收敛通道 `fe_take_branch`。当该脉冲有效时，顶层同拍执行两件事：一是广播 `squash` 到所有 `PipelineConnect` 以及 F1/Fp 内部状态寄存器，使在途项同步失效；二是将 `pc_q` 从默认的 `pc_q + 4` 改写为 `io.predict.dnpc`，使下一拍 F0 直接从预测目标重新取指。这样，控制冒险不必等待后端回写权威 PC，即可在前端就地止损，将 taken 分支的气泡压缩到 2 拍。
...
关于分支预测命中率、前端 flush 事件分解及其与三组基准程序（MicroBench / Dhrystone / CoreMark）的分析，详见 @sec:perf-fe-breakdown 与 @tab:perf-bp。
...
译码级接收前端送入的指令字与 PC，通过 CU（Control Unit）解码出指令类型、功能单元选择、操作码、源/目的寄存器编号等控制信号，通过 IGU（Immediate Generate Unit）提取并符号扩展立即数。译码级的输出形成完整的微操作描述，经流水线寄存器#footnote[该流水线寄存器采用前文 `PipelineConnect` 实现，见 @pipeline-connect。]传递给重命名级。
...
这套以全局 fence 换取正确性的策略并非没有代价：每次磁盘块传输都触发一次整 cache 刷写，DCache 的时间局部性被完全击穿，对磁盘访问密集型负载（如文件系统压测、Shell 启动）的性能表现会显著恶化。更理想的做法是在硬件侧引入跨主设备的缓存一致性协议，例如类似 MESI #cite(<papamarcos1984mesi>) 的嗅探（snoop）机制：DMA 发起访存时，Crossbar 以 bus snooping 将事务广播至 DCache，由 DCache 根据本地 tag 与行状态决定是转发脏数据、降级本地状态还是直接放行；CPU 写命中共享行时主动发出 invalidate 广播，使外部主设备感知。这样软件就无需在每次 MMIO 前后粘贴 fence，一致性由硬件按行粒度自动维护，对缓存局部性的损伤也被限制在真正发生冲突的单个缓存行上。这一方向涉及 AXI4-ACE #cite(<arm2023ace>) / CHI #cite(<arm2023chi>) 等一致性互连协议与多主设备仲裁的重新设计，已超出本文单核教学目标的范围，留作未来工作。
...
软件侧 UART 初始化如 @uart-init 所示。这里 soc 侧的分频系数 `div = 1`，只需让 NVBoard 满足相同的采样频率即可正常收发字符。
...
#figure(
  ...
  align(center)[
    #table(
      ...
      [2 000 轮],
      [由 Weicker 于 1984 年提出的综合整数基准；默认 500 000 轮在 RTL 仿真下不切实际，本实验将 `NUMBER_OF_RUNS` 减至 2 000，保留基准程序的循环结构完整性],

      [CoreMark #cite(<coremark2009>)],
      [20 iterations],
      [EEMBC 工业标准嵌入式基准，包含链表操作、矩阵乘法与状态机三类核心算法；默认 1 000 iterations 过大，本实验压至 20 iterations 以便在 10 分钟内完成],
    )
  ],
...
三组负载的 IPC 分别为 0.47、0.44 与 0.56，CPI 对应为 2.12、2.27 与 1.79；IPC 随负载特性分层出现，其中 CoreMark 由于存在较多长直线段和极高的 ICache 命中率（见 @tab:perf-icache），可被流水前端连续供应指令，因而取得最高的 IPC。MicroBench train 规模下子测试运行时间足够长，BP 与 Cache 都得到了充分预热，IPC 已略超 Dhrystone。

从前后端协同角度看，可以引入一个反映前后端协同效率的指标 `commit / ifu_deliver`（`ifu_deliver` 见 @tab:perf-fe-events）：其中 `ifu_deliver` 是前端 IFU 在 F3 段成功交付给后端的指令数（即"塞入"指令队列的有效指令数），`commit` 则是后端 ROB 最终顺序提交并写回架构状态的指令数。两者之差对应"被前端发出、但因分支误预测、异常或 `fence.i` / CSR 写等 serializing 事件而中途被流水线 flush 掉的指令"，因此该比值在理想情况下应趋近于 1，比值越接近 1 说明前端推测发出的指令越少被作废，前后端的协同效率越高。

本实现下该比值稳定在 $approx$ 0.88，说明每 100 条由前端送往后端的指令中约有 12 条最终被作废。结合本处理器的流水线深度可对这一损耗给出合理解释：前端 F1/F2/Fp/F3 共 4 级，后端经译码、重命名、分派、发射、执行、写回到提交共约 7 级，整条流水线深度达到 4+7 级；当后端在提交点确认一次分支误预测时，CommitStage 通过 `io.flush` 在一个周期内同时清空前端 IFU 的所有流水段、指令队列以及后端从重命名到写回的所有流水段，单次 flush 损失的有效工作量近似等于流水线深度。该比值反映了"全流水线 flush"恢复策略放大了每次分支误预测的代价。

要进一步提升该比值，可考虑降低单次 flush 的惩罚：例如分支预测出错误，可以在执行阶段就对前端做重定向，而不是要等到提交时再对前端做重定向，降低分支指令带来的惩罚。

== 前端事件分析 <sec:perf-fe-breakdown>

为进一步定位前端中各段的开销，@tab:perf-fe-events 列出了前端 PerfEvent 在三组负载上的计数。事件含义与指标定义见 @tab:perf-metrics。需要说明的是，本实验运行于 Abstract Machine 裸机环境，未配置页表也未启用 Sv39 分页，iTLB 查表全程被旁路，因此 F1 TLB-miss 计数对所有负载均为 0，下表不再单独列出该列。
...
#figure(
  ...
  align(center)[
    #table(
      columns: 5,
      align: (left, right, right, right, right),
      table.header([基准], [IFU deliver], [fe\_redirect branch], [F2 ICache wait], [Fp resp wait]),
      [MicroBench (train)], [83 659 693], [8 982 270], [20 745 873], [6 818],
      [Dhrystone (2 000)], [1 117 693], [161 648], [403 268], [1 825],
      [CoreMark (20 iter)], [8 107 703], [856 609], [1 187 045], [3 996],
    )
  ],
) <tab:perf-fe-events>
...
+ ICache 是首要前端停顿源：`F2 ICache wait` 在所有负载上都是前端停顿周期的主导项。其中 MicroBench train 的 ICache 等待周期达到 20.7M，占总周期的 13.3%；与 IFU 总停顿比 32.06%（49.9M / 155.6M 周期）之比 $approx$ 0.42，说明 ICache 等待是前端停顿的主导但非全部，剩余部分由 Fp 段排队、后端反压与重定向气泡构成。这一观察表明前端剩余性能主要受制于缓存与主存之间的传输带宽，而非流水线结构本身。
+ Fp 响应等待极少：`Fp resp wait` 在所有负载上均不到 7 000 周期，表明在典型负载下 ICache 的访问延迟大多可以被流水覆盖，Fp 作为"in-flight 缓冲段"承担了绝大多数 cache 响应周期的吸收工作。
...
#figure(
  ...
  align(center)[
    #table(
      ...
      [MicroBench (train)], [11 282 371], [1 018 777], [90.97%], [1 018 780],
...
+ MicroBench train 规模包含 10 个结构各异的 micro-kernel，单个子测试的动态指令量级已达 7M+，GShare 历史寄存器能够在每个子测试内部充分收敛；但子测试之间结构差异（排序、A\* 搜索、解释器、压缩、哈希等）仍带来一定预测器抖动，整体命中率为 90.97%，略低于循环密集的 Dhrystone 与 CoreMark。

进一步对比流水线 flush 次数（1 018 780 / 10 538 / 77 719）与误预测数可以发现，两者在三组负载上的差值均仅为 3，基本呈一一对应关系。这说明本处理器的实现严格做到了"一次误预测触发一次流水线冲刷"，未出现因前端状态残留而产生的多余 flush；同时也印证了分支预测器采用的提交点更新策略能够有效避免投机路径上的分支信息提前污染预测器状态。整体而言，本实现的 GShare + IJTC + RAS 组合在三组负载上的命中率均处于 90% 以上的区间，与同类配置的 GShare 在 SPEC CPU2006 #cite(<henning2006spec2006>) 整数子集上 88%–92% 的平均命中率水平相当，符合主流教学及研究实现的预期。
...
DCache 的命中率整体仍然较高，但 MicroBench train 的 96.87% 明显低于另外两组。该差异主要源于工作集大小的悬殊差异：MicroBench 的多数子测试都会在堆上分配大于 DCache 容量的数据结构（参见 MicroBench 文档，`qsort`/`sieve`/`15pz`/`dinic`/`lzip`/`ssort`/`md5` 等子测试 train 档堆区使用量级在 64 KB 至数 MB 之间，远超 4 KiB DCache 容量），并对其进行近似随机的访问，产生大量容量缺失与冲突缺失；而 Dhrystone 与 CoreMark 的工作集均小于 4 KiB（Dhrystone 主循环活跃数据仅有几个全局结构体与字符串缓冲区，CoreMark 默认 `TOTAL_DATA_SIZE` 为 2 000 字节的链表/矩阵/状态机数据缓冲区），因此命中率显著更高。

DCache 的平均缺失代价（21–34 周期）整体高于 ICache（约 18–20 周期），源于 DCache 支持写回策略——当替换脏行时需要先发起一次 AXI4 写 Burst，再启动读 Burst 补齐新行，较 ICache 的单向读 Burst 多一次往返延迟。
...
#figure(
  ...
  align(center)[
    #table(
      ...
      align: (left, right),
      table.header([基准], [得分]),
      [Dhrystone (DMIPS/MHz)], [0.517],
      [CoreMark (CoreMark/MHz)], [1.540],
    )
  ],
) <tab:perf-industrial>

具体地，Dhrystone：$2000 times 10^6 / (2 thin 201 thin 801 times 1757) approx 0.517$ DMIPS/MHz；CoreMark：$20 times 10^6 / 12 thin 983 thin 050 approx 1.540$ CoreMark/MHz。

*与 ARM Cortex-M 系列对照。* 作为参考，ARM 官方 Cortex-M 处理器对照表 #cite(<arm2024cortexm3>) 给出 Cortex-M0 约 0.84 DMIPS/MHz，Cortex-M3/M4 约 1.25 DMIPS/MHz，Cortex-M7 约 2.14 DMIPS/MHz。本处理器 Dhrystone 成绩 0.517 DMIPS/MHz 低于 Cortex-M0 档位，主要原因有三：

+ 稳态 IPC 仍处 0.44 区间——单发射 in-order 流水线 + ICache miss 主导前端停顿，留下了较大的 IPC 提升空间；
...
+ 三组负载 IPC 分别为 0.47 / 0.44 / 0.56，CPI 对应为 2.12 / 2.27 / 1.79；前后端指令守恒关系 `commit / ifu_deliver ≈ 0.88` 表明前端与后端共同参与了瓶颈构成；
...
+ 分支预测在循环密集负载（Dhrystone）上达到 94.64% 的高命中率，在异质 micro-kernel 集合（MicroBench train）上为 90.97%，三组负载命中率均在 90% 以上，整体表现与同类 GShare 方案的主流水平相当；
+ ICache 在三组负载下均取得 99.99% 以上命中率、AMAT 接近 1 周期；DCache 除 MicroBench train 因子测试堆区工作集普遍大于 4 KiB DCache 容量而降至 96.87% 外，其余负载均超过 99.7%，Cache 子系统表现稳定；
+ 工业指标方面，Dhrystone 达到 0.517 DMIPS/MHz、CoreMark 达到 1.540 CoreMark/MHz，处于教学级单发射 in-order RISC-V 实现的合理区间；与商用 ARM Cortex-M 系列的差距主要源于稳态 IPC 仍处 0.44 区间、编译器优化等级（`-O2`）以及未对 Dhrystone 库函数做专门优化，未来通过双发射前端、流水化乘除与 PGO 等手段仍有显著提升空间。
