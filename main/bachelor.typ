...
#show: doc.with(
  ...
  conclusion: [
    ...
    + 缓存子系统设计。实现了 VIPT 结构的 ICache 与 DCache，均为 4 路组相联、64 组、64 字节缓存行，采用 Pseudo-LRU 替换策略。处理器核心侧采用自定义类 SRAM 总线协议降低接口复杂度，缓存对外通过 AXI4 Burst 完成 refill 与 writeback。DCache 支持写回策略、Write Buffer 优化与 fence 全缓存刷写。
    ...
    本文工作的局限性与未来改进方向包括：当前处理器仍为单发射结构，指令吞吐率受限于每周期至多提交一条指令，后续可扩展为双发射或多发射以进一步提升性能，届时 BTB 与多端口 BP(branch predict) 也将成为必要的补充；当前前端将 Predict 模块挂在 F3 的组合路径上，这在未来追求更高时钟频率时会成为时序关键路径，可通过将 BPU 拆出独立流水段缓解；当前缓存层次仅有 L1，可增加 L2 缓存以降低主存访问延迟；目前 DMA 读写内存前后需要插入 `fence` 使得整片缓存无效，未来可引入支持缓存一致性总线协议。此外，可进一步完成 FPGA 综合与上板验证，评估实际时钟频率与资源占用。本文所述设计已在 GitHub 上以 GPLv2 协议开源 #footnote[项目仓库：#link("https://github.com/KINGFIOX/ysyx-workbench")[https://github.com/KINGFIOX/ysyx-workbench]]。
  ],
  ...
  digital-signature-option: (
    ...
    scanned-copy: [
      ...
      // #import "@local/muchpdf:0.1.0": muchpdf
...
// 三线表辅助函数：将传入的 table 参数自动转换为三线表样式。
// 约定：第一个位置参数为 table.header(...)，其余位置参数为单元格内容。
#let 三线表(..args) = {
  let cells = args.pos()
  table(
    ..args.named(),
...
如 @fig:soc-arch 所示，SoC 整体互连分为三个速率域。处理器核心作为唯一的 AXI4 Master 接入一级 AXI4 Crossbar。一级 Crossbar 的左侧分支连接 SDRAM Controller，右侧分支经 AXI4 Fragmenter（将突发传输拆分为单拍事务以适配不支持 Burst 的从设备）与 AXI4 UserYanker（剥离用户自定义信号以对齐接口宽度）连接二级 AXI4 Crossbar。SDRAM 直接挂载在一级 Crossbar 上，以最短路径获得最高带宽。
...
二级 Crossbar 下挂 MROM（mask rom，掩膜只读记忆体）、SRAM（低延迟片上存储）以及 AXI4ToAPB 总线桥。总线桥将 AXI4 事务转换为 APB 协议后，由 APB Fanout 按地址空间分发至各低速外设：XIP Flash Controller、PSRAM QSPI Master、UART 16550、GPIO、PS2 键盘控制器、VGA 显示控制器，以及 PLIC（外部中断控制器）和 CLINT（定时器与软件中断控制器）。

在 Flash 访问路径上，XIP Flash Controller 与 APB Fanout 共享底层 SPI Controller 的物理访问权，通过 SPI Arbiter 进行仲裁：XIP 以高优先级发起 SPI 事务用于直接取指执行，APB 以低优先级提供 SPI 寄存器的直接读写通道。各外设控制器的数据端口连接至 NVBoard 虚拟开发板对应的外设仿真（串口终端、LED/数码管、键盘、VGA 显示器），以虚线表示。
...
外设侧围绕“可用、可测、可观测”的思想进行设计。UART、GPIO、PS2、VGA 等控制器通过 APB 域接入，保证接口一致性与调试便利性。针对教学与开发场景，系统进一步集成 NVBoard，将串口回显、按键输入、数码管显示与 VGA 输出纳入同一仿真闭环，使 SoC 级功能验证从单纯日志比对扩展为可交互的行为验证。
...
Flash 存储器通过 SPI 协议接入系统。SPI Master 选用 OpenCores 开源的 #cite(<opencores_simple_spi>)、基于 Wishbone 总线接口 #cite(<wishbone_b4>)的 IP 核，本课题使用 Chisel 实现了 APB-to-Wishbone 的薄封装层以接入 SoC 的 APB 域。

系统为 Flash 提供两种访问方式。第一种是命令方式：通过 APB Fanout 经 SPI Arbiter 直接读写 SPI 控制器寄存器，适用于 Bootloader 阶段的 Flash 编程与配置操作，如 @fig:flash-read-sw 所示。第二种是 XIP（eXecute In Place）方式：XIP 控制器将处理器对 Flash 地址窗口的 MMIO 读请求自动转换为底层 SPI 命令字序列（如读命令、地址发送与数据回传），从而以硬件方式完成“取指驱动”，其状态机流程如 @fig:xip-fsm 所示。引入该机制的动机是避免启动阶段的“鸡生蛋”死锁：若程序存放在 Flash 中而访问 Flash 又依赖软件驱动，则驱动本身也需要先从 Flash 取出，系统将无法启动。通过 XIP 的硬件读通路，处理器无需先运行 SPI 软件驱动即可直接从 Flash 取指执行。两种方式通过 SPI Arbiter 共享物理 SPI 控制器，XIP 具有更高的仲裁优先级。
...
本章介绍了 SoC 中三个关键存储控制器的设计与验证。Flash 控制器通过 SPI Arbiter 同时支持命令访问与 XIP 直接取指执行；PSRAM QSPI Master 以 FSM + Clgen + Shift 三模块架构实现 QSPI/QPI 协议兼容；SDRAM 控制器通过 Chisel 重写并实现了流水化、位拓展与低位交叉字扩展。三个控制器均配有行为级仿真模型并通过协议级验证，具备直接对接物理芯片的能力。
...
本小结介绍了前后端解耦的微架构。前后端通过握手协议相连、指令队列作为缓冲层，使得前端分支预测器或取指流水线的升级不影响后端接口，有利于微架构的增量迭代与独立优化。
...
如 @fig:frontend-arch 所示，IFU 通过 ICache 端口和 PTW(page table walker) 端口分别访问指令缓存与页表，取到的指令与 PC 传递给 Predict 进行分支预测，Predict 返回预测的下一条 PC 供 IFU 在下一轮取指时使用。IFU 的输出（指令、PC、预测信息）经指令队列送入后端。后端在分支预测失败或异常恢复时，通过 redirect 信号更新 Predict 的预测器状态，并通过 flush 和 dnpc 信号使 IFU 从正确地址重新取指。

== 取指单元（IFU）设计 <sec:ifu-pipeline>

=== 总体架构

为在保持单发射单提交约束的前提下充分释放后端乱序执行潜能，IFU 采用 F0、F1、F2、Fp、F3 的流水结构，其设计目标是在直线代码上达到稳态 IPC=1，taken 分支仅付出 2 个气泡周期。整体架构与各段职责如 @fig:ifu-pipeline 所示。整个 IFU 由顶层模块与四个子模块（`F1Stage`、`F2Stage`、`FpStage`、`F3Stage`）构成，分别承担 PC 生成、地址翻译、ICache 控制、分支预测与后端交付的职责。
...
除 F2 $->$ Fp 这一路外，其余子模块均通过 `PipelineConnect` 串联。`PipelineConnect` 本质上是一个参数化流水级：它在上下游之间插入带握手的 valid 寄存器和 `bits` 锁存寄存器；当 `thisIn.ready` 为高时，接收上一拍的 `prevOut.valid / bits`；当 `flush` 有效时，清零 valid 并丢弃当前槽位。稳态下，流水按 `ready-valid` 协议持续推进；发生 `flush` 时，可在同一拍清空所有中间的数据槽。实现上通过 Scala 泛型参数 `T <: Data` 抽象载荷类型，使其既能承载 IFU 内部的 `F1In`、`F2In`、`F3In` 等 Bundle，也能复用于后端分派、发射与执行阶段，从而保持全项目流水接口风格一致：
...
F0 位于 IFU 顶层，不作为独立子模块存在，只维护架构 PC 寄存器 `pc_q`。在每个周期，F0 都投机地按 `pc_q + 4` 推进到 F1。若 F3 确认出现 taken 分支（如直接跳转、被预测为跳转的条件分支等），前端会立即执行自重定向，将 `pc_q` 拉回预测目标。除此之外，凡是需要改写 PC 的情况（分支误预测、trap、xret、`fence`、`sfence.vma`、CSR 写等），统一由后端通过 `flush` 携带正确的 `dnpc`(Dynamic Next Program Counter, dnpc) 来重写 `pc_q`。顶层还会生成全局 squash 信号 `squash = fe_take_branch ⋁ flush`，并广播到所有 `PipelineConnect` 的 `flush` 端口，以及 F1 与 Fp 的内部状态寄存器，从而保证整条流水在同一拍内同步清空。F1、F2+Fp、F3 三段的具体实现见下文。
...
PTW 子 FSM 是整个 F1 中唯一的状态机，需要与后文 @sec:ptw 的 PTW 内部九态遍历 FSM 区分：前者是 F1Stage 内部用于驱动 PTW 请求/响应握手并与上层 squash 协同的外层 FSM，后者才是真正执行三级页表遍历的内部算法状态机。该外层 FSM 的主路径为 `idle -> req -> waitResp -> idle`，并在此基础上补充 squash 处理，其中 `idle`、`req`、`waitResp` 分别表示：空闲态、等待与 `ptw` 握手成功、等待 `ptw` 将查询结果返回；此外 `drain` 表示：排空正在进行的遍历页表事务。为此，F1 维护了 @fig:ifu-ptw-fsm 所示的四状态握手 FSM，并配合两个辅助寄存器 `ptw_vpn_reg` 与 `ptw_abandoned` 记录在途请求上下文。
...
先看正常路径：`idle -> req -> waitResp -> idle`。在空闲态下检测到 `tlb` 未命中后，FSM 进入 `req`，等待 PTW 接收新事务；当与 `ptw` 成功握手后，锁存当前 VPN(virtual page number)，并转入 `waitResp`；当页表遍历完成，FSM 回到空闲态。此时只有同时满足“未被放弃、无 fault“成立时，才会真正 refill iTLB；否则将该响应视为过期并静默丢弃，避免错误 PPN 写入 iTLB。

再看 squash 路径（@fig:ifu-ptw-fsm 中虚线）。若 squash 发生时总线上已有在途页表读请求（例如 `req` 已握手成功，或已进入 `waitResp` 但响应尚未返回），则转入 `drain`，后续仅静默吞掉响应，不 refill、也不报告 fault。其余情况下（如 `req` 尚未握手，或 `waitResp` 与响应同拍到达），说明不存在未决事务，可直接回到空闲态。`drain` 本质上是纯等待态：一旦收到响应即返回 `idle`。
...
F2 是纯组合段，不持有寄存器。它根据 `F2In` 驱动 `req / addr / size` 并完成 `req / ack` 握手；地址按指令字长对齐，请求仅在“上游有效、非故障、且 Fp 有空槽”时发起，避免无效占用 ICache 带宽。对 F1 上报的 page fault（页故障），F2 走“故障旁路“直送 F3、绕过 Fp（故障不应占用 inflight 槽位）。“cache 路径“与“故障旁路“最终由“故障旁路“优先多路选择汇合为同一份 `F3In`，再经流水段进入 F3。

Fp 是 IFU 中承载多条“在途的缓存读事务“的核心结构，实现为深度 4 的环形队列。采用 4 深度而非单项缓冲，是为了匹配 ICache 连续命中时“每拍可接收新请求”的流水能力，避免 F2 过早反压。Fp 使用三个指针：`enq_ptr`（F2 入队）、`deq_ptr`（F3 出队）和 `rdata_ptr`（响应回填）；前两者通过翻转位区分满/空，`rdata_ptr` 指向待回填的槽位，在每次缓存读事务完成时，从 `icache` 返回的数据写入 `rdata_ptr` 指向的槽位，并且 `rdata_ptr` 回绕自增 。

Fp 的 squash 策略比 F1 更直接：`squash = 1` 同拍复位三个指针。此后即使旧响应返回，该旧响应也会被忽略，不再写入槽位。

=== F3 级：预测与后端交付

F3（`F3Stage`）是整条 IFU 的交付点。结构上，F3 本身是纯组合逻辑，唯一的时序元件来自其输入侧的段流水寄存器。F3 的职责主要有三项：切出指令字、驱动分支预测器、生成前端自重定向脉冲。

若上游表项已标记发生了取指异常，则 F3 强制将指令字置 0，并在 `IFUOutput` 中输出 `mcause = 12`、`mtval = pc`、`has_except = 1`，供后端在 ROB 提交点统一触发 trap。

分支预测器端口以 `pc`、`inst` 为输入，以 `dnpc`、`ghr`(global history register) 为输出；F3 在同一拍将预测结果与原始指令一起封装到 `IFUOutput`。
...
为此，F3 额外提供一条基于预测结果的前端收敛通道 `fe_take_branch`，当 `fe_take_branch=1` 时，以意味着：分支预测器预测的指令地址不会顺序自增。因为前面的流水段都是假设指令地址是顺序的，此时已经有预测的错误路径进入了取指流水线，需要被清空。此时，顶层同拍执行两件事：一是广播 `squash` 到所有段流水寄存器以及 F1/Fp 内部状态寄存器，使在途项指令失效；二是将 `pc_q` 从默认的 `pc_q + 4` 改写为 `predict.dnpc`，使下一拍 F0 直接从预测目标重新取指。这样，控制冒险不必等待后端回写权威 PC，即可在前端就地止损，将 taken 分支的气泡压缩到 2 拍。

这里的“推测性”在于：这次回卷只依赖前端预测。若预测错误，后端在若干拍后发现 `redirect.dnpc` 与 `predict.dnpc` 不一致，再由 CommitStage 发出 `flush`，用权威 `dnpc` 修正 `pc_q`。在实现上，`pc_q` 的更新采用优先多路复用，优先级为“后端权威 PC > 前端预测 PC > 顺序推进(PC+4)”。

同理，前端不为这类会影响 PC 的 serializing 指令（如修改 `mtvec`/`mepc`、`xret`、`ecall`、`ebreak`）建立独立预测通道。主要矛盾在于：这些指令的下一 PC 依赖后端执行结果，前端无法在 F3 提前给出可靠预测；在实现上等价于先按顺序路径 `pc + 4` 推进，随后由后端在检测到真实控制流后发出 `flush` 进行纠正。该策略虽会引入额外回卷，但在此类指令出现频率较低、专用硬件收益有限（Amdahl 定律 #cite(<amdahl1967validity>)）的前提下，仍是复杂度与收益更平衡的选择。
...
在取指流水化方面，IFU 采用 F0、F1、F2、Fp、F3 的流水结构，以"直线代码稳态 IPC=1、taken 分支仅引入 2 拍气泡"为性能目标。F1 作为 MMU 级完成 iTLB 查询与 PTW 慢路径遍历；F2 与 Fp 共同构成 ICache 控制级，F2 负责请求通道、Fp 以深度 4 的环形 FIFO 追踪在途响应，二者间以直连代替 `PipelineConnect` 以满足 ICache 握手时序；F3 作为提交点在一拍内完成指令切分、预测驱动与后端交付。
...
在推测性控制冒险处理上，前端通过 F3 的 `fe_take_branch` 脉冲就地收敛 taken 分支，将气泡压缩到 2 拍。除此之外，所有需要前端回卷的场景统一交给后端 CommitStage 发出的 `flush` 处理，其触发可分为以下两类：

+ “改 PC”事件，包括分支误预测、trap 与 xret：后端确定权威目标后，以 `flush` + `dnpc` 对 `pc_q` 做优先级多路选择。

+ “影响前端状态但不一定改 PC”的事件，包括 `fence`（cacheline 失效）、`sfence.vma`（iTLB 表项失效）以及改变 `satp`/特权级的 CSR 写：即便 PC 可保持顺序，前端仍需同拍清空所有 inflight 事务，并按后端给出的 `dnpc`（通常为 `pc+4`）重新取指。
...
译码级接收前端送入的指令字与 PC，通过 CU（Control Unit）解码出指令类型、功能单元选择、操作码、源/目的寄存器编号等控制信号，通过 IGU（Immediate Generate Unit）提取并符号扩展立即数。译码级的输出形成完整的微操作描述，经流水线寄存器#footnote[该流水线寄存器采用前文 `PipelineConnect` 实现，见 @fig:pipeline-connect。]传递给重命名级。
...
FreeList。FreeList 管理空闲物理寄存器池。重命名级从中分配新物理寄存器，提交级向其归还不再需要的物理寄存器。归还的依据是：当一个逻辑寄存器被后续指令重新定义（即发生 WAW）时，承载旧值的物理寄存器的生命周期随之结束，可以被安全释放。以 @tbl:rename-prd-lifecycle 指令序列为例：
...
#figure(
  align(center)[
    #三线表(
      columns: 5,
      align: center,
      table.header([序号], [指令], [逻辑 rd], [分配 prd], [old\_prd]),
      [1], [ADD x1, x2, x3], [x1], [p10], [p1],
      [2], [SUB x1, x4, x5], [x1], [p11], [p10],
    )
  ],
  kind: table,
  supplement: [表],
  caption: [寄存器重命名中物理寄存器分配与回收示例],
) <rename-prd-lifecycle>
...
+ 将指令写入 ROB，获取 ROB 标签作为指令在乱序流水线中的唯一标识。
...
\5. 处理 fence 与 sfence.vma：提交这两类指令时，分别向外输出 fence\_i 和 sfence\_vma 信号，通知 ICache 失效或 TLB 刷新。
...
CSRU 内部实现了完整的 M-mode 与 S-mode CSR 寄存器集合，如 @tbl:csr-ms-regs 所示：
...
#figure(
  ...
  kind: table,
  supplement: [表],
  caption: [CSRU 支持的 M/S 模式 CSR 寄存器],
) <csr-ms-regs>
...
中断的注入发生在 CommitStage：当 ROB 头部指令正常提交（无异常）且 CSRU 报告有待处理的中断时，CommitStage 先正常提交当前指令（更新 ArchRAT、释放 old\_prd），随后立即触发中断 trap。中断的 epc(exception program counter) 设置为当前提交指令的下一条 PC（retiring\_dnpc），使中断返回后可从正确位置继续执行。
...
如 @fig:ptw-fsm 所示，PTW 从 idle 状态接受请求后，依次访问第 2 级、第 1 级与第 0 级页表项。每一级的访问地址由上一级 PTE 中的 PPN 拼接当前级的 VPN 索引与 3 位字节偏移构成（每个 PTE 为 8 字节）。每级读取 PTE 后检查有效位（PTE.V）：若无效则进入 fault 状态报告页错误；若有效则继续下一级。遍历成功后进入 done 状态，输出叶子 PTE 的 PPN 与 flags 供 TLB refill 使用。
...
处理器核心（IFU、LSU）与缓存之间的交互采用自定义的类 SRAM（SRAM-like）总线协议（协议接口如 @sram-bundle 所示），而非直接使用 AXI4 #cite(<arm2020axi4>) 等复杂片上总线。这一设计的出发点是降低处理器核心侧的接口复杂度：AXI4 协议包含 5 个独立通道（AR、AW、W、R、B）与多种突发传输模式，对于处理器核心每次仅发起单拍读写的场景过于冗余；而类 SRAM 协议将读写统一到一组信号上，核心侧仅需驱动一个请求端口即可完成访存操作。
...
如 @fig:plru-access 所示，每次缓存命中时根据命中的路号更新树节点标志，使其指向远离被访问路的方向。如 @fig:plru-victim 所示，需要替换时沿树节点标志位从根到叶遍历，得到的叶子即为牺牲路。该算法仅需 3 位状态与简单的组合逻辑即可实现 4 路近似 LRU 替换，硬件开销远低于完整 LRU 所需的访问序记录。
...
如 @fig:icache-fsm 所示，ICache 的主状态机在原先五个状态的基础上，为支持前端对 `fence` 的支持额外引入了 `fence_drain` 状态，共六个状态：
...
+ fence_drain：专用于"AXI burst 已启动、但 fence 已到达"的中间状态。进入该状态后 ICache 消费完来自总线的突发读事务，但不再将数据回写到 tag RAM / cacheline，也不推进 PLRU，直到 `r.last` 抵达后回到 idle。

#figure(
  image("images/icache-fsm.png"),
  caption: [ICache 主状态机（虚线为 fence 相关跃迁）],
  supplement: [图],
) <icache-fsm>

围绕 `fence` 的处理贯穿了所有与总线相关的状态。当 `fence` 脉冲到达时，ICache 会在同一拍把 `tag_ram(*).valids` 全部清零——这使整个缓存在"语义"上立即变空。剩下的问题是如何与"已启动、无法撤销"的 AXI 事务共存：

+ 在 idle / lookup / miss 三个状态下，ICache 尚未在 AXI 上发起任何 AR 握手，因此 `fence` 到来时直接回到 idle；已失效的 tag RAM 保证下一次查询必然 miss 并重新从总线取指。
+ 在 replace 状态下，AR 通道的 `valid` 已经高起但是否握手取决于下游 `ready`。若 `fence` 与 `ar.fire` 恰好同拍，则本次 burst 已经不可撤销，必须跳入 `fence_drain` 把后续 R 拍全部吞掉；否则只需下一拍直接回 idle，AR 随之解挂。
+ 在 refill 状态下，AR 必然已经 fire、R 通道正在返回，此时 `fence` 强制进入 `fence_drain`。此外还存在一种极端时序：若 `fence` 与 `r.last` 同拍到达，常规 refill 路径会在本拍把该行写入 tag RAM 并置 valid。为此，本设计采用优先级多路选择： “`fence` clear valids“ > “`r.last` set valid”。

这样，无论 `fence` 在哪一个周期抵达，ICache 都能在至多一次额外 burst 的代价内回到"完全空缓存 + AXI 无事务"的干净状态，且整个过程不需要前端参与握手。前端因此可以对 `fence` 完全保持透明：它像普通指令一样被取到 F3、送入后端，由后端 CommitStage 在提交时统一发出 `fence` 脉冲与 `io.flush`， 其中 `io.flush` 负责同周期清空整条取指流水。
...
+ idle：等待请求。收到 `fence` 时进入 flush 流程；正常请求时锁存地址与写控制信号（wen、wdata、wstrb），进入 lookup。`sfence.vma` 仅作用于地址翻译缓存（TLB），不直接触发 DCache 刷写。
...
+ flush\_scan -> flush\_wb：由 `fence`（或缓存维护触发）进入全缓存刷写流程。flush\_scan 逐组逐路扫描，遇到脏行则进入 flush\_wb 通过 AXI4 写回该行，写回完成后清除脏位并继续扫描，直到所有组路遍历完毕回到 idle。
...
一个典型的实例是 Bootloader 的快速启动路径：系统上电后，处理器从 XIP（Execute-In-Place）模式的 Flash 开始执行，Bootloader 将 Flash 中的应用程序映像搬运到 SDRAM 以获得更高的访问速度，随后跳转至 SDRAM 入口地址继续执行。其核心指令序列如 @tbl:fencei-wrong-seq 所示：
...
RISC-V 指令集架构通过 `fence` 指令显式解决这一问题。正确的指令序列如 @tbl:fencei-correct-seq 所示：
...
#figure(
  ...
  kind: table,
  supplement: [表],
  caption: [执行 fence 后的正确指令序列],
) <fencei-correct-seq>

在本处理器的实现中，`fence` 的执行分为两个阶段：
...
经过 `fence` 的两阶段处理，步骤 1 写入 DCache 的指令数据已安全落入 SDRAM，而 ICache 的全部内容已失效。当步骤 3 跳转到 `sdram_addr` 时，ICache 必然未命中，从 SDRAM 重新加载到正确的指令内容，程序得以正确执行。

需要指出的是，`fence` 是一条代价较高的指令：它要求刷写整个 DCache 并失效整个 ICache，开销与缓存容量成正比。RISC-V 规范将其定义为"Zifencei"扩展而非基础指令集的一部分，正是考虑到在多核系统中维护指令-数据一致性有更高效的硬件机制（如 RISC-V CMO 扩展 #cite(<riscv2023cmo>)）。在本处理器的单核场景下，`fence` 足以保证正确性，且实现简洁。
...
+ CPU $->$ DMA 方向：xv6 在发起磁盘写请求前，先将待写入的数据填入驱动描述符指向的数据缓冲区；这条 store 指令经 DCache 采用的写回策略被吸收为脏行，尚未通过 AXI4 写通道落回 SDRAM。此时若 CPU 继续向 virtio 的 MMIO 通知寄存器写入 "queue notify"，DMA 便会立即从 SDRAM 读取数据——读到的却是 DCache 背后的旧值，磁盘最终落盘的是错误内容。
+ DMA $->$ CPU 方向：磁盘读完成后 DMA 将数据直接写入 SDRAM 的目标缓冲区，并通过中断或 virtio 的 used ring 通知 CPU。若 CPU 在此前曾访问过同一地址范围，DCache 中已缓存了旧数据，后续的 load 指令将命中 DCache 而根本看不到 DMA 刚刚写入的新数据，内核读到的依然是未更新前的内容。
...
本章围绕缓存子系统展开，首先介绍了面向处理器核心的类 SRAM 总线协议，其 req-ack-done 三信号握手机制在保持接口简洁的同时支持读地址与读数据的流水分离；随后描述了基于 Diplomacy 框架的参数协商与 SRAMToAXI4 总线桥的设计，使 ICache 与 DCache 对内呈现 SRAM 接口、对外通过 AXI4 Burst 完成 refill 与 writeback。在替换策略方面，采用 Pseudo-LRU 算法以 3 位状态实现 4 路近似最近最少使用替换，兼顾硬件面积与替换效果。ICache 采用六状态有限状态机实现只读缓存逻辑，其中专用的 `fence_drain` 状态将 `fence` 与 inflight 的 AXI burst 的收敛完全封闭在缓存内部，使前端不再需要为 `fence` 维护额外的握手或粘滞位；DCache 则通过七状态有限状态机支持写回策略、Write Buffer 优化及全缓存刷写。最后讨论了分离式 ICache 与 DCache 架构下的指令-数据一致性问题，说明了 `fence` 指令通过 DCache 刷写加 ICache 失效两阶段操作保证自修改代码的正确执行；以及接入 virtio-disk 后 DMA 与 DCache 之间的可见性缺口，说明了本文以 MMIO 前后软件 fence 规避该问题的工程取舍，并指出引入类 MESI 的硬件一致性协议是更理想的长期方向。
...
Sdb 提供类 GDB 的交互式命令行界面，基于 GNU Readline 实现命令补全与历史记录。支持的调试命令如 @tbl:sdb-commands 所示：
...
+ FTrace(Function Tracer) 从 ELF 文件的符号表中加载函数名与地址映射，在运行时根据指令特征识别函数调用与函数返回，维护调用深度计数器，生成带缩进的调用链记录。FTrace 对操作系统启动流程的调试尤为有用：当程序陷入死循环或异常跳转时，调用链能清晰展示控制流的偏离点。
...
程序员编写软件时只需关心 ISA 状态——这正是 ISA 作为硬件与软件之间契约的核心含义。即便存在诸如 `fence` 这样涉及微架构行为的指令，其语义仍然是在 ISA 层面定义的（"使后续取指看到此前的数据写入"），而非暴露具体的缓存实现细节。
...
+ DUT 侧：`VerilatorCpu::step()` 驱动时钟直到 CommitStage 提交。通过探针读取 DUT 的 PC、下一 PC（`dnpc`）、机器码、通用寄存器与 CSR——这些正是 ISA 状态的完整快照。
...
+ 异常检测：当仿真检测到异常状态（如 `VerilatorCpu::step()` 超过最大周期数仍未收到 CommitStage 的提交信号，表明总线可能死锁），触发快照回放机制。
...
软件侧 UART 初始化如 @fig:uart-init 所示。这里 soc 侧的分频系数 `div = 1`，只需让 NVBoard 满足相同的分频系数即可正常收发字符。
...
从前后端协同角度看，可以引入一个反映前后端协同效率的指标 `commit / ifu_deliver`（`ifu_deliver` 见 @tbl:perf-fe-events）：其中 `ifu_deliver` 是前端 IFU 在 F3 段成功交付给后端的指令数（即"塞入"指令队列的有效指令数），`commit` 则是后端 ROB 最终顺序提交并写回架构状态的指令数。两者之差对应"被前端发出，但因分支误预测、异常或 `fence` / CSR 写等 serializing 事件而中途被流水线 flush 掉的指令"，因此该比值在理想情况下应趋近于 1，比值越接近 1 说明前端推测发出的指令越少被作废，前后端的协同效率越高。
